...
<img src="https://github.com/Raycosine/raycosine.github.io/blob/master/_images/hypercube.jpg" width="30%" style="margin: auto 0;">

The adjacency matrix is \\(A=\sum_{j=1}^n \sigma_x^(j)\\)

When \\(G\\) is regular, \\(L=A+deg\times I\\), the walk generated by \\(L\\) have an overall phase \\(e^{-i deg\ t})\\ compared to the one generated by \\(A\\).
...
Recall that for any \\(p\\), the state \\(\|\lambda\rangle\\) with amplitudes \\(\langle col j\|\lambda\rangle = e^{ipj}\\) satisfies \\(\langle col j\|A\|\lambda\rangle = e^{ipj}=\lambda \langle col j\|\lambda\rangle = e^{ipj}\\), where \\(\lambda=2\sqrt{2} \cos{p}\\) for all values of \\(j\\) except \\(0,n,n+1,2n+1\\). The eigenvalue condition (\\(\langle col j\|A\|\lambda\rangle = e^{ipj}=\lambda \langle col j\|\lambda\rangle = e^{ipj}\\)) for \\(j=0,2n+1\\) can be satisfied by replacing \\(\|\lambda\rangle\\) by linear combination of \\(\{\|\lambda\rangle\}\\) previously used. And then we obtain (16.51)~(16.53).
